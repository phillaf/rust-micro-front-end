...
mod tests {
    ...
    async fn setup_test_app() -> axum::Router {
        ...
        // Reset metrics registry before creating the app to avoid AlreadyReg errors
        crate::metrics::AppMetrics::reset_registry();
    ...
    async fn test_display_username_existing_user() {
        ...
        let request = Request::builder().uri("/display/username/admin").body(Body::empty()).unwrap();
        ...
        // Get body content
        let body_bytes = extract_body_bytes(response.into_body()).await;
    ...
    async fn test_display_username_invalid_username() {
        ...
        let request = Request::builder().uri("/display/username/a").body(Body::empty()).unwrap();
    ...
    async fn test_api_username_endpoint() {
        ...
        let request = Request::builder().uri("/api/username/admin").body(Body::empty()).unwrap();
    ...
    async fn test_static_endpoints() {
        ...
        // Test sitemap.xml
        let request = Request::builder().uri("/sitemap.xml").body(Body::empty()).unwrap();
        let response = app.clone().oneshot(request).await.unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        let body_bytes = extract_body_bytes(response.into_body()).await;
        let body_str = std::str::from_utf8(&body_bytes).unwrap();
        ...
        // Test manifest.json
        let request = Request::builder().uri("/manifest.json").body(Body::empty()).unwrap();
        let response = app.oneshot(request).await.unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        let body_bytes = extract_body_bytes(response.into_body()).await;
        let body_str = std::str::from_utf8(&body_bytes).unwrap();
    ...
    async fn test_edit_endpoint_unauthorized() {
        ...
        let request = Request::builder().uri("/edit").body(Body::empty()).unwrap();
