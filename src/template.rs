...
impl TemplateService {
    ...
    pub fn new(cache_enabled: bool, minify_enabled: bool) -> Result<Self> {
        ...
        info!(
            "Template service initialized with caching: {}, minification: {}",
            cache_enabled, minify_enabled
        );
    ...
    fn get_cached_html(&self, key: &str) -> Option<String> {
        self.template_cache.read().ok().and_then(|cache| cache.get(key).cloned())
    }
    ...
    fn minify_html(&self, html: &str) -> Result<String> {
        ...
        String::from_utf8(minified).map_err(|e| anyhow::anyhow!("Failed to convert minified HTML to string: {}", e))
    }
